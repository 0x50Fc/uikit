...
namespace wx {
    ...
    */
    export function createAudioContext(id: string, thisObj: any): AudioContext {
        return
    }
    /**
    ...
    */
     ...
     * 参数
     */
    export function pauseVoice(callbacks?: wx.callback_success_fail_complete) { }

    /**
     * 开始播放语音。同时只允许一个语音文件正在播放，如果前一个语音文件还没播放完，将中断前一个语音播放。
     */

    export function playVoice(obj: (res: playVoiceObj) => any) { }
    /**
     * 结束播放语音。
     */
    export function stopVoice(callbacks?: wx.callback_success_fail_complete) { }

    /**
 * filePath	string		是	需要播放的语音文件的文件路径	
 * duration	number	60	否	指定录音时长，到达指定的录音时长后会自动停止录音，单位：秒	>= 1.6.0
 */
    interface playvoiceParam extends wx.callback_success_fail_complete {
        filePath: string,
        duration?: number
    }
    /**
    ...
    interface setInnerAudioOptionParam extends wx.callback_success_fail_complete {
    ...
    interface playVoiceObj extends wx.callback_success_fail_complete {
