...
namespace wx {
     ...
     * 获取文件信息
     */
    export function getFileInfo(obj: fileInfoObj) { }
    ...
    export function getSavedFileInfo(obj: saveFileInfoObj) { }
    /**获取该小程序下已保存的本地缓存文件列表 */
    export function getSavedFileList(obj: saveFileListObj) { }
    /**新开页面打开文档 */
    export function openDocument(obj:openDocumentObj) { }
    /**
     * 删除本地缓存文件
     */
    export function removeSavedFile(obj:removeSavedFileObj){}
    /**
     * 保存文件到本地。注意：saveFile 会把临时文件移动，因此调用成功后传入的 tempFilePath 将不可用
     */
    export function saveFile(obj:saveFileObj){}


    interface fileInfoObj {
        filePath: string
        digestAlgorithm?: string
        success: (res: fileInfoSuccessObj) => any
        fail?: (res: failCallbackRes) => any
        complete?: (res: completeCallbackRes) => any
    }
    interface fileInfoSuccessObj {
        size: number
        digest: string
    }

    interface saveFileInfoObj {
        filePath: string
        success: (res: saveFileInfoSuccessObj) => any
        fail?: (res: failCallbackRes) => any
        complete?: (res: completeCallbackRes) => any
    }
    interface saveFileInfoSuccessObj {
        size: number	//文件大小，单位 B	
        createTime: number	//文件保存时的时间戳，从1970/01/01 08:00:00 到该时刻的秒数
    }

    interface saveFileListObj {
        success: (res: saveFileListSuccessObj) => any
        fail?: (res: failCallbackRes) => any
        complete?: (res: completeCallbackRes) => any
    }

    interface saveFileListSuccessObj {
        fileList: Array<fileItemObj>	//文件数组，每一项是一个 FileItem
    }
    interface fileItemObj {
        filePath: string	//本地路径	
        size: number	//本地文件大小，以字节为单位	
        createTime: number	//文件保存时的时间戳，从1970/01/01 08:00:00 到当前时间的秒数
    }
    interface openDocumentObj extends wx.callback_success_fail_complete{
